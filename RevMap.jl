...
for date in 1:num_continuum_dates
	for delay in 1:num_tdf_times
		interpolation_points[date,delay]=continuum_dates[date]-tdf_times[delay]
  end
...
while it <= nits && converged==0        #ADMM ITERATION LOOP
  ...
  #for l in 1:1
    ...
    # Evaluation:
      if l == 350
	ext = 1.0/2 * (H*vec(X[it,l,:])-vec(L[l,:]))'*squeeze(W[l,:,:],1)*(H*vec(X[it,l,:])-vec(L[l,:]))
        println("Line = ", l, "Chi2 = ",Chi2(Model((X[it,l,:]),ICF),L[l,:],EL[l,:]), " Mat = ",ext)
        #println("Rho: ",rho[it,l]," U: ",mean(U[it-1,l,:]))

	M = Model(squeeze(X[it,l,:],1),ICF)
	B =  H*vec(X[it,l,:])
	  println("X: ",size(vec(X[it,l,:])))
